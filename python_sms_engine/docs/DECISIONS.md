...
## Modem Discovery Decisions (FINAL)

### ❌ Rejected
- /dev/serial/by-id (unstable for identical modems)
- hash(port) identity (breaks on USB reorder)
- full ttyUSB scanning (slow, not scalable)

### ✅ Accepted

#### Sysfs-based detection
- Source of truth: /sys/class/tty/*
- Stable physical grouping

#### Interface strategy
- if02 → primary
- if03 → fallback
- do NOT probe all interfaces

#### SMS capability check
- AT+CPMS required
- AT/CPIN/CREG alone are NOT enough
