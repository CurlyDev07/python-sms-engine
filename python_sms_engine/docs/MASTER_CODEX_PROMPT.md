...
## SMS Engine Rules (CRITICAL)

- NEVER scan all ttyUSB ports during send
- ALWAYS use registry lookup first
- modem identity MUST be SIM-based (IMSI)
- if02 is primary port
- if03 is fallback ONLY

### Discovery Rules
- Use sysfs grouping
- Probe only if02
- Validate with AT+CPMS

### Send Rules
- No discovery during send
- No full scan fallback
- Only retry + fallback port